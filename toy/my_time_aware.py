...
def get_awake_time():
    ...
    """
    with open(
        os.path.join(os.path.dirname(__file__), "awake_time.txt"), "r", encoding="utf-8"
    ) as f:
        awake_time = f.read()
    try:
...
def set_awake_time(ask):
    ...
    with open(
        os.path.join(os.path.dirname(__file__), "awake_time.txt"), "w", encoding="utf-8"
    ) as f:  # 对了就写入
        f.write(input_time)
        print("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n修改成功")
    return True
...
def calculate_and_show():
    ...
    awake_duration = now - datetime.datetime.combine(
        datetime.datetime.now().date(), awake_time
    )  # 计算清醒时长
    ...
    bed_time = (
        datetime.timedelta(days=1) - bed_to_sleep - sleep_duration - awake_duration
    )
