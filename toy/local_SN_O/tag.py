...
def extract_tags_and_hierarchy(items):
    ...
    for tag_id, title in tags.items():
        ...
        while current in child_to_parents:
            parent_id = child_to_parents[current][0]  # Assuming only one parent for simplicity
            current = parent_id
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Add tags to markdown files and organize files based on tags.")
    parser.add_argument('--input', type=str, default='decrypted-sn-data.txt', help='Path to the JSON file containing the notes and tags')
    parser.add_argument('--output', type=str, default='sn_output', help='Path to the output folder where markdown files are stored')
